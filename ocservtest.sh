...
Add_iptables(){
        ...
        iptables -A INPUT -i ${Network_card} -p esp -j ACCEPT
...
Set_iptables(){
	...
	if [[ -z ${ifconfig_status} ]]; then
        ...
        iptables -t nat -A POSTROUTING -s 10.31.1.0/24 -o ${Network_card} -j MASQUERADE
        iptables -t nat -A POSTROUTING -s 10.31.2.0/24 -o ${Network_card} -j MASQUERADE
